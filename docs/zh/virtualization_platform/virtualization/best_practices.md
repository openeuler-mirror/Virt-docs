...
目前openEuler 21.03 及以上版本中提供了libtpms和swtpm的源，可以直接使用yum命令安装。
...
1. 虚拟机配置文件中添加如下配置：
    ...
    > [!NOTE]说明
    >
    > 目前，openEuler 20.09 版本 AArch64 架构上的虚拟机可信启动不支持 ACPI 特性，所以虚拟机请勿配置 ACPI 特性，否则启动虚拟机后无法识别 vTPM 设备。AArch64 架构在openEuler 22.03 LTS 之前的版本，tpm model 配置为 \<tpm model='tpm-tis-device'\>。
